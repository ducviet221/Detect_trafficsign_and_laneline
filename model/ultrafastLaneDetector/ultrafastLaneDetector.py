...
from model.ultrafastLaneDetector.model import parsingNet

lane_colors = [(0,0,255),(0,255,0),(255,0,0),(0,255,255)]

# khởi tạo vị trí làn đường dựa vào dataset
...
class UltrafastLaneDetector():
	...
	def initialize_model(model_path, cfg, use_gpu):
		...
		# đánh giá net được chỉ định có hợp pháp hay không rồi thực thi
		net.eval()
	...
	def process_output(output, cfg):		
		...
		# lấy xác suất điểm đặc trưng nhât
		prob = scipy.special.softmax(processed_output[:-1, :, :], axis=0)
		# khởi tạo vị trí griding
		idx = np.arange(cfg.griding_num) + 1
		# Chuyển griding thành dạng mảng từng điểm
		idx = idx.reshape(-1, 1, 1)
		# Lấy ra xác suất của các vị trí trong griding
		loc = np.sum(prob * idx, axis=0)
		# Tìm vị trí của điểm có giá trị lớn nhất trong mảng
		processed_output = np.argmax(processed_output, axis=0)
		# Loại bỏ giá trị của các điểm có giá trị lớn nhất
